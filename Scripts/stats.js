...
// Función para calcular el total donado a todas las organizaciones
function calculateTotalDonations(donors) {
    return donors.reduce((total, donor) => total + parseFloat(donor.amount), 0);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Calcular el total donado a todas las organizaciones
    const totalDonations = calculateTotalDonations(donorsList);

    // Mostrar el total en la página
    const totalDonadoElement = document.getElementById('totalDonado');
    if (totalDonadoElement) {
        totalDonadoElement.textContent = `$${totalDonations.toLocaleString()}`;
    }
});
